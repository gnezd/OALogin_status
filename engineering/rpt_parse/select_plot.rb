...
def select_uv_curve_plot(path, svg_out, n, name)
	...
	puts "find \"#{path}\" -maxdepth 1 -name \"#{name}.rpt\" -mtime -6 -type f -printf '%Ts %f\n'| sort -nr|head -#{n}|cut -d ' ' -f2-"
	fname_list = []
	time_list = []
	puts rpt_list.size
	...
	rpt_list.each do |fname|
		...
		rpt.samples.reverse.each do |sample|
			...
			curve = sample.pressure_curve
			max = sample.max_pressure
			raise "no curve in #{sample.name}" if curve.size == 0

			curve.each do |pt|
				plot_data.puts "#{pt[0]}\t#{(pt[1].to_f)*max/100}" #real_intensity
